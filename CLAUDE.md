...
1. Create `src/steps/yourstep.jl` with:
   ...
   - Dispatch method: `analyze(smld::BasicSMLD, cfg::YourStepConfig; kwargs...)` that times the call and returns `(result, StepInfo(...))`. The `kwargs...` is required so the pipeline can pass context keywords that the step may not use.
