...
- `precision` — lateral localization precision in microns, tested as
  `max(σ_x, σ_y)`. These σ are the CRLB-based fit uncertainties (Mortensen et
  al. 2010); a precision ceiling is the most direct way to bound reconstruction
  resolution.
- `pvalue` — goodness-of-fit p-value; a floor rejects fits whose residuals are
  inconsistent with the PSF model (often multi-emitter or contaminated ROIs).
...
- K. I. Mortensen, L. S. Churchman, J. A. Spudich, H. Flyvbjerg. "Optimized
  localization analysis for single-molecule tracking and super-resolution
  microscopy." *Nature Methods* **7**, 377–381 (2010).
  [doi:10.1038/nmeth.1447](https://doi.org/10.1038/nmeth.1447) — the basis of the
  CRLB localization-precision (`σ`) values used by the `precision` criterion.
