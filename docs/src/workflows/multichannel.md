...
A multi-channel analysis has two phases:
...
You describe both phases with one `MultiTargetConfig` and one `analyze` call.

## Configuring `MultiTargetConfig`

`MultiTargetConfig` ties the channels together. Its fields:
