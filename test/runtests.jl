...
@testset "fast" begin
    ...
    @testset "install_agent_guide" begin
        ...
        mktempdir() do dir
            ...
            # plus the manifest listing exactly those generated names.
            @test length(refs) == 12
            @test ".smlma-manifest" in refs
        ...
        # user adds inside reference/ is never touched, but files we generated (and the
        # manifest tracking them) ARE regenerated by a refresh / removed by uninstall.
        mktempdir() do dir
            skill  = install_agent_guide(dir = dir)
            ...
            @test !isfile(joinpath(refdir, ".smlma-manifest"))  # manifest removed
            @test !isfile(joinpath(skill, "SKILL.md"))
            @test !agent_guide_status(dir = dir).installed
        end
        ...
        mktempdir() do dir
            ...
            @test isdir(refdir)
            @test !isfile(datamd)
            @test !isfile(joinpath(refdir, ".smlma-manifest"))
        ...
        mktempdir() do dir
            ...
            @test length(readdir(joinpath(bundle, "reference"))) == 12
